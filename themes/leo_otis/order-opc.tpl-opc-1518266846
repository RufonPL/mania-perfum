...
{else}
	...
	<div class="col-xs-12 col-md-6">
        ...
        <p class="carrier_title">{if $lang_iso=="pl"}{l s='Zostaw wiadomość'}{else}{l s='Leave a message'}{/if}</p>
	 			<div>
	 				
	 				<textarea class="form-control" cols="120" rows="2" name="message" id="message" placeholder="{if $lang_iso=="pl"}{l s='Jeżeli chcesz dodać komentarz do swojego zamówienia, wpisz go poniżej.'}{else}{l s='If you would like to add a comment about your order, please write it in the field below.'}{/if}">{strip}
		...
		<!-- END Payment -->
			...
			{if $conditions AND $cms_id}
				<p class="carrier_title">{if $lang_iso=="pl"}{l s='Warunki korzystania z usług.'}{else}{l s='Terms of Service.'}{/if}</p>
				<div class="alert alert-danger" id="errorCGV" style="display: none">{if $lang_iso=="pl"}{l s='Musisz zaakceptować regulamin'}{else}{l s='You must accept the terms and conditions'}{/if}</div>
				<p class="checkbox">
					<input type="checkbox" name="cgv" id="cgv" value="1" {if $checkedTOS}checked="checked"{/if} />
					<label for="cgv">{if $lang_iso=="pl"}{l s='Zgadzam się z Warunkami Świadczenia Usługi i podporządkuję się im bezwarunkowo.'}{else}{l s='I agree to the terms of service and will adhere to them unconditionally.'}{/if}</label> 
					<a href="{$link_conditions|escape:'html':'UTF-8'}" class="iframe" rel="nofollow">{if $lang_iso=="pl"}{l s='(przeczytaj)'}{else}{l s='(Read the Terms of Service)'}{/if}</a><span class="validation-state"></span>
				</p>
				
			{/if}		
		
		<div style="margin: 0 0 20px 0; text-align: center; width: 100%;">
			<a id="submitOrder" class="btn btn-default button button-medium"><span>{if $lang_iso=="pl"}{l s='Potwierdź zakup'}{else}{l s='Orders'}{/if}</span></a>
		</div>
		</div>
        </div>
