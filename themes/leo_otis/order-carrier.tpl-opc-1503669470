												...
												<td>												
													<label for="delivery_option_{$id_address}_{$option@index}">
														{if $option.unique_carrier}
															...
															{foreach $option.carrier_list as $carrier}
																<b>{$carrier.instance->name}</b>
															{/foreach}
															{if isset($carrier.instance->delay[$cookie->id_lang])}
																<br/>{$carrier.instance->delay[$cookie->id_lang]}
															{/if}
														{/if}
														...
														<label for="delivery_option_{$id_address}_{$option@index}">
															{if $option.total_price_with_tax && !$option.is_free && (!isset($free_shipping) || (isset($free_shipping) && !$free_shipping))}
																{if $use_taxes == 1}
																	...
																	{if $priceDisplay == 1}
																		{convertPrice price=$option.total_price_without_tax} <span>{l s='(tax excl.)'}</span>
																	{else}
																		{convertPrice price=$option.total_price_with_tax} <span>{l s='(tax incl.)'}</span>
																	{/if}
