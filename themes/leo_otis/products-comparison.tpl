						...
						<div class="product-image-container image">
							...
							<div class="group-label">
									...
									{if isset($product->specificPrice) && $product->specificPrice}
										{if {$product->specificPrice.reduction_type == 'percentage'}}
											<div class="price-percent-reduction-box label-price-percent-reduction label">
												<span class="price-percent-reduction product-label">
								...
								{if isset($product->new) && $product->new == 1}
									<a class="new-box label label-new" href="{$product->getLink()|escape:'html':'UTF-8'}">
										<span class="product-label">{* {l s='New'} *}New</span>
									</a>
								{/if}
								{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
									{if $product->on_sale}
										<a class="sale-box label label-sale" href="{$product->getLink()|escape:'html':'UTF-8'}">
											<span class="product-label">{* {l s='Sale!'} *}Sale!</span>
										</a>
									{/if}
								{/if}
							</div>
							...
							{if $product->on_sale}
									{elseif $product->specificPrice AND $product->specificPrice.reduction}
										<div class="product_discount">
											<span class="reduced-price label label-danger">{l s='Reduced price!'}</span>
										</div>
									{/if}
