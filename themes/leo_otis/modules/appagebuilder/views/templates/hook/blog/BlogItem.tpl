...
<div class="blog-container" itemscope itemtype="https://schema.org/Blog">
    ...
    <div class="right-block">
    	{if isset($formAtts.bleoblogs_scre) && $formAtts.bleoblogs_scre}
			<div class="created">
				<time class="date" datetime="{strtotime($blog.date_add)|date_format:"%Y"}{*convert to date time*}">
					<!-- day of month -->
					<span class="day">{l s=strtotime($blog.date_add)|date_format:"%d" mod='appagebuilder'}</span>
					<span class="month">{l s=strtotime($blog.date_add)|date_format:"%b" mod='appagebuilder'},</span>
					<span class="year">{l s=strtotime($blog.date_add)|date_format:"%Y" mod='appagebuilder'}</span>
				</time>
			</div>
		{/if}
		<div class="blog-meta">
			...
			<span class="author">
				...
				{l s='Post by' mod='appagebuilder'}: <span class="meta-value">{$blog.author|escape:'html':'UTF-8'}</span>
			</span>
			{/if}
			
			{if isset($formAtts.bleoblogs_scat) && $formAtts.bleoblogs_scat}
			<span class="cate">
				<i class="cate-icon icon-paper-stack"></i>{l s='Category' mod='appagebuilder'}: 
			...
			<span class="nbcomment">
				<i class="comment-icon icon-speech-bubble"></i>{l s='Vỉew' mod='appagebuilder'}: <span class="meta-value">({$blog.comment_count|intval})</span>
			</span>
			{/if}
			...
			<span class="hits">
				...
				{l s='Hit' mod='appagebuilder'}: <span class="meta-value">({$blog.hits|intval})</span> 
			</span>	
			{/if}
		...
		{if isset($formAtts.show_desc) && $formAtts.show_desc}
	        ...
	        <p class="btn-more">
	        	<a href="{$blog.link}{*full url can not escape*}" class="more" title="{$blog.title|escape:'html':'UTF-8'}">{l s='Read more' mod='appagebuilder'}</a>
	        </p>
        {/if}
    </div>
</div>
