...
{if isset($p) AND $p}
			...
			<form class="showall" action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get">
				<div>
	                ...
	                <button type="submit" class="btn btn-default button exclusive-medium">
	                	<span>{l s='Show all' mod='leoproductsearch'}</span>
	                </button>
	...
	<div class="product-count">
        ...
        {/if}
        {if $nb_products > 1}
        	{l s='Showing %1$d - %2$d of %3$d items' mod='leoproductsearch' sprintf=[$productShowingStart, $productShowing, $nb_products]}
		{else}
        	{l s='Showing %1$d - %2$d of 1 item' mod='leoproductsearch' sprintf=[$productShowingStart, $productShowing]}
       	{/if}
    </div>
{/if}
